...
`emit.core`/`emit.rules` internals in particular) is private and may
...
- Split the cooperative SIMD-group/MMA GEMM lowering (`emit/coop.py`,
  `emit/gemm.py`), flash attention, custom_vjp, and MLP-training
  examples and tests out into a new sibling project, `mgemm`. That
  capability had grown into a self-contained, still-below-JAX-CPU-parity
  GEMM compiler in its own right (hand-tuned SIMD-group geometry
  selection, MMA fragment tiling, cross-instance rhs staging) and was
  dominating this project's scope; palladium returns to its original
  focus, hand-authored ODE/SDE integrator kernels over `fori_loop`/
  `scan`/`while_loop` control flow.
- `emit_msl`, `dispatch.bind`, `KernelDiagnostics`, and `MetalCallable`/
  `FfiCallable` lose the `cooperative` parameter/field and the
  model-selection logic entirely; every kernel now lowers through the
  one-thread-per-instance ("classic") model, the only one that remains.
  `explain()` reports launch geometry and MSL size, not an execution
  model choice.
- See `mgemm`'s own README and `docs/notes/` for the GEMM-side history;
  its `docs/notes/*.md` files carry provenance notes back to this
  project's commits at the fork point.
