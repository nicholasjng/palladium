...
@pytest.mark.parametrize(
    "steps,interval", [(1, 1), (7, 1), (7, 3), (7, 10), (23, 5), (100, 10)]
)
...
def test_all_six_vjp_inputs(steps, interval, seed):
    ...
    cotangents = tuple(
        jnp.asarray(rng.normal(size=n), dtype=jnp.float32) for _ in range(2)
    )
...
def test_forward_and_configuration():
    ...
    np.testing.assert_allclose(
        jax.jit(solve)(*args), reference_solve(*args, steps=7), rtol=2e-5
    )
