...
def test_transpose_matches_numpy(rng):
    ...
    f = palladium.metal_call(kernel, out_shape=jax.ShapeDtypeStruct((6, 4), jnp.float32))
    got = f(x)
...
def test_rhs_transposed_matmul_matches_numpy(rng):
    ...
    f = palladium.metal_call(kernel, out_shape=jax.ShapeDtypeStruct((8, 4), jnp.float32))
    got = f(dy, w)
