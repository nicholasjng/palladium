...
def trees(n_inputs: int, compare_refs: range | None = None):
    ...
    if compare_refs is None:
        ...
        cmp_refs = st.sampled_from(list(compare_refs)).map(lambda i: Leaf(ref=i, lit=None))
    ...
    return st.recursive(
        ...
        lambda kids: st.one_of(
            st.tuples(st.sampled_from(sorted(UNARY)), kids).map(lambda t: Node(t[0], (t[1],))),
...
def input_arrays(count: int):
    return st.tuples(
        *[arrays(np.float32, (N,), elements=st.floats(-2.0, 2.0, width=32)) for _ in range(count)]
    )
...
def scan_cases(draw):
    ...
    ys_plans = draw(
        st.tuples(
            *[st.tuples(trees(n_vals, compare_refs=stable), st.booleans()) for _ in range(n_ys)]
        )
    )
...
def make_scan_kernel(
    ...
    def kernel(*refs):
        ...
        def body(carry, x):
            ...
            for plan in carry_plans:
                if isinstance(plan, int):
                    ...
                    new.append(jnp.tanh(eval_tree(plan, vals) + 0.0 * carry[0]))
            ys = tuple(jnp.tanh(eval_tree(plan, vals) + 0.0 * carry[0]) for plan, _ in ys_plans)
        ...
        for out, value in zip(outs[:n_carry], final, strict=True):
            out[...] = value
        if ys_plans:
            for (_, direct), out, ys_value in zip(ys_plans, outs[n_carry:], stacked, strict=True):
...
def make_switch_kernel(branches, n_inputs):
    ...
    def kernel(*refs):
        *ins, out = refs
        ...
        res = jax.lax.switch(
            idx,
            [lambda tree=tree: eval_tree(tree, vals) + 0.0 * vals[0] for tree in branches],
        )
        out[...] = res
