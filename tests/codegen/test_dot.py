...
def test_square_matmul_matches_numpy(rng):
    ...
    f = palladium.metal_call(_matmul_kernel, out_shape=jax.ShapeDtypeStruct((8, 8), jnp.float32))
    got = f(a, b)
...
def test_rectangular_matmul_matches_interpret(rng):
    ...
    f = palladium.metal_call(_matmul_kernel, out_shape=jax.ShapeDtypeStruct((4, 10), jnp.float32))
    got = f(a, b)
...
def test_non_square_inner_dim_matches_numpy(rng):
    ...
    f = palladium.metal_call(_matmul_kernel, out_shape=jax.ShapeDtypeStruct((5, 3), jnp.float32))
    got = f(a, b)
    np.testing.assert_allclose(got, a @ b, rtol=1e-4, atol=1e-4)
...
def test_batched_matmul_is_rejected():
    def kernel(a_ref, b_ref, o_ref):
        o_ref[...] = jax.lax.dot_general(a_ref[...], b_ref[...], (((2,), (1,)), ((0,), (0,))))
