...
@pytest.mark.parametrize(
    "steps,interval,width", [(1, 1, 2), (5, 2, 2), (5, 9, 4), (16, 4, 4)]
)
def test_checkpointed_cnf_vjp(steps, interval, width):
    ...
    weights = jnp.asarray(
        rng.normal(0, 0.3, (n, parameter_count(width))).astype(np.float32)
    )
    ...
    for variant, spacing in [
        ...
        solve = make_flow(
            n, steps=steps, interval=spacing, width=width, variant=variant
        )
