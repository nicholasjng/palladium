...
def vdp_adaptive_kernel(x0_ref, v0_ref, mu_ref, to_ref, xo_ref, vo_ref, steps_ref, rejected_ref):
...
def _mu_ensemble(rng, n: int, stiff_fraction: float = 0.0) -> np.ndarray:
    ...
    if stiff_fraction:
        ...
        mu[rng.choice(n, k, replace=False)] = rng.uniform(300.0, 500.0, k).astype(np.float32)
    return mu
