...
matching test failed): explain() model flipped ("cooperative" for
classic), rejection reason dropped (None for classic), dtype check
removed from BoundKernel.launch (silent cast came back), rank-1
canonicalization disabled (contraction EmitError). Exception: removing
the promote cast in the scalar dot does NOT currently fail the
accumulation test, because the Metal compiler contracts
`float_acc += half * half` into a float fma anyway (verified against
the sabotaged MSL); the test pins the required behavior against future
compilers, and the cast makes the semantics explicit in the source.
"""
...
def _coop_call():
    def kernel(q_ref, k_ref, o_ref):
        o_ref[...] = jnp.dot(q_ref[...], k_ref[...].T)

    return palladium.metal_call(kernel, out_shape=_shaped(8, 64))
...
def test_explain_cooperative():
    diag = _coop_call().explain(_shaped(8, 64), _shaped(64, 64))
    ...
    assert "model=cooperative" in str(diag)


def test_explain_classic_names_the_rejection():
    diag = _classic_call().explain(_shaped(8, 8), _shaped(8, 8))
    assert diag.model == "classic"
    assert diag.rows is None
    assert diag.reason == "dot_general rhs is not a device view"
    assert diag.threadgroup is None
    assert 'reason="dot_general rhs is not a device view"' in str(diag)


def test_explain_explicit_threadgroup_opts_out():
    def kernel(q_ref, k_ref, o_ref):
        o_ref[...] = jnp.dot(q_ref[...], k_ref[...].T)

    call = palladium.metal_call(kernel, out_shape=_shaped(8, 64), threadgroup=64)
    diag = call.explain(_shaped(8, 64), _shaped(64, 64))
    assert diag.model == "classic"
    assert diag.reason == "explicit threadgroup opts out of the cooperative model"
...
def test_explain_ffi_matches_metal_call():
    ...
    call = palladium.metal_call_jit(kernel, out_shape=_shaped(8, 64))
    diag = call.explain(_shaped(8, 64), _shaped(64, 64))
    assert diag.model == "cooperative"
    assert diag.rows == 8
...
def test_palladium_explain_env_logs_once_per_compile(monkeypatch, capsys, rng):
    ...
    assert err.count("model=classic") == 1
    assert "dot_general rhs is not a device view" in err
...
def test_preferred_element_type_accumulates_in_f32():
    ...
    # m=7 does not divide the SIMD-group, pinning the classic scalar
    # path (the cooperative dot stages its own float conversion).
    def kernel(a_ref, b_ref, o_ref):
    ...
    assert call.explain(a, b).model == "classic"
    got = call(a, b)
