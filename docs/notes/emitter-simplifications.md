...
- **Alignment follow-ups now possible** (`CVal.align` is a gcd, not a
  ...
  assertions in wider loads, and 8/16-element checks if wider vector
  types ever matter.

## Note (2026-08-19): the cooperative-model items formerly here

This file used to also track observability and design gaps in a
simdgroup-cooperative/MMA execution model (fallback-cliff diagnostics,
`cond`/`while` under cooperative layout, batched `dot_general`, a
scalar/cooperative model unification). That model was split out into a
separate project, `mgemm`, along with this file's copy predating this
edit (see its `docs/notes/` for the original, unabridged version and
provenance note). Removed here since it no longer describes any code
in this repo.
...
Watch item: any future loop-lowering code with the same
three-or-more-array-temps-beside-a-carry-swap shape should get an
oracle test first, since the fuzzer only found this one by chance.
